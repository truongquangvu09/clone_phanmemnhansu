...
export default function DataQTLV({ list_provision, dataCheckBox ,localListCheck}) {
  const [listCheck, setListCheck] = useState<any>([]);
  const data = list_provision?.data;
  ...
  useEffect(() => {
    setListCheck(localListCheck.list_provision || []);
  }, [localListCheck]);
  return (
            ...
            <div key={item.id} className={`${styles.show}`}>
              ...
              <td className={`${styles.show_checkbox}`}>
                <input
                  ...
                  defaultValue={listCheck.includes(item.id) || localListCheck?.list_provision}
                  checked={listCheck.includes(item.id)  || localListCheck?.list_provision}
